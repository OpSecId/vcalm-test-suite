...
- Queries sharing the same `group` value are processed as AND; queries with different or missing `group` values are processed as OR. *(Descriptive protocol rule; no RFC 2119 keyword in source prose — excluded from `tests/normative-statements.js`.)*
